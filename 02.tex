...
\todo{What means simple addressing? Describe it in section in-core execution, cite, cite hofmann-2016-hsw and intel, here reference to that description}.
\todol{AGU: base plus index plus offset, simple AGU: base plus offset}
...
for the FMA (port $0$ and $1$), $12$\,cy for the twenty four loads (port $3$ and $4$),
and $8$\,cy for the eight stores (port $5$).
$32$ addresses need to be generated. We assume the simple AGU on port $7$ is not used, as we observed for DAXPY (section \ref{sec:epm}). Generating all $32$ addresses on AGUs on ports $3$ and $4$ requires $16$\,cy.
...
%  t_\text{L1} = \max(12\,\cyw, 14\,\cyw, 8\,\cyw, 4\,\cyw) =
  t_\text{L1} = \max(12\,\cyw, 16\,\cyw, 8\,\cyw, 0\,\cyw) =
16\,\cyw.
\ee
%
...
\todol{which is in line with the measurement in figure}
Fig.~\ref{fig:daxpy-indirect:perf}.
...
Transferring a cache line between L1/L2 and L2/L3 takes $1$\,cy. Hence it takes \todol{check 1 cy and the results below}
%
\be
