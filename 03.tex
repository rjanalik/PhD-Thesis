...
In this chapter, we analyze the data transfer between the different cache levels and the FLOPs performed during the sparse triangular solve phase. The results are used as input for our performance model to be established \todol{in the next section}. Therefore, we inspect the different instantiations of the loops resulting from Algorithm~\ref{alg:algo:fw} and \ref{alg:algo:bw}. \todol{Our assumption is}
...
\section{Application of Modified Roofline Model on Sparse Triangular Solve}
\label{sec:mrm}
